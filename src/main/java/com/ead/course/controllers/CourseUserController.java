...
public class CourseUserController {
    ...
    public ResponseEntity<Object> getAllUsersByCourse(@PageableDefault(page = 0, size = 10, sort = "userId", direction = Sort.Direction.ASC)Pageable pageable,
        ...
        log.info("getAllUsersByCourse - Consultando os usuários relacionado ao curso");
    ...
    public ResponseEntity<Object> saveSubscriptionUserInCourse(@PathVariable(value = "courseId")UUID courseId,
        ...
        try {
            ...
            log.info("Locating the User in the User Microservice.");
            if(responseUser.getBody().getUserStatus().equals(UserStatus.BLOCKED)){
                log.info("User for subscription is blocked.");
    ...
    public ResponseEntity<Object> deleteCourseUserByUser(@PathVariable(value = "userId") UUID userId){
        if(!courseUserService.existsByUserId(userId)){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("CourserUser not found.");
        }
        courseUserService.deleteCourseUserByUser(userId);
        log.info("CouserUser {} deleted successfully.", userId);
        return ResponseEntity.status(HttpStatus.OK).body("CouserUser deleted successfully.");
    }
